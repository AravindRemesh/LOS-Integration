...
#Read SAV file
data = readsav(r"C:\Users\aravi\Downloads\cme_nofil_noheat_gcsfit_VAR80 (1).SAV")
x = data['r']/(7*(10**10))
y = data['th']
z = data['phi']
rho = data['rho']
...
y2 = []
z2 = []
for p in np.linspace(min(l),max(l),200):
    for r in np.linspace(min(m),max(m),200):
        for q in np.linspace(min(k),max(k),100):
            x_1.append(q)
        X_1.append(x_1)
        y2.append(p)
        z2.append(r)
...
rho_int_f =[]
x_max = len(X_1)*len(X_1[0])
for p in range(0,x_max,len(X_1[0])):
    rho_int_f.append(rho_int[p:p+len(X_1[0])]*f(T_int_log[p:p+len(X_1[0])]))
...
for p in range((len(X_1))):
    int_value.append(np.trapz(rho_int_f1[p],X_1[p]))

int_value = np.array(int_value)

plt.scatter( y2 , z2 , c = int_value, cmap = 'plasma' )
plt.colorbar(label='Integral Rho^2.R(T).dx')
plt.xlabel('Y')
plt.ylabel('Z')
plt.show ()
