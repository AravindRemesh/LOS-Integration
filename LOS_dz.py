...
for p in np.linspace(min(k),max(k),200):
    for q in np.linspace(min(l),max(l),200):
        for r in np.linspace(min(m),max(m),100):
            z_1.append(r)
        Z_1.append(z_1)
        x2.append(p)
        y2.append(q)
...
rho_int_f =[]
z_max = len(Z_1)*len(Z_1[0])
for p in range(0,z_max,len(Z_1[0])):
    rho_int_f.append(rho_int[p:p+len(Z_1[0])]*f(T_int_log[p:p+len(Z_1[0])]))

rho_int_f = np.array(rho_int_f)
rho_int_f1 = rho_int_f**2

int_value =[]
for p in range((len(Z_1))):
    int_value.append(np.trapz(rho_int_f1[p],Z_1[p]))

int_value = np.array(int_value)

plt.scatter( x2 , y2 , c = int_value, cmap = 'plasma' )
plt.colorbar(label='Integral Rho^2.R(T).dz')
plt.xlabel('X')
plt.ylabel('Y')
plt.show ()
